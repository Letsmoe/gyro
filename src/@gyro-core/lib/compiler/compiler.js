...
class Compiler {
    ...
    emit_move(reg, expr) {
        if (expr) {
            ...
            if (expr.type === ResultType.RBP_OFFSET) {
                this.emit(`mov ${reg}, DWORD [rbp${positive}${expr.value.offset}]`);
            }
    ...
    emit_expr(expr, typeConversion = null, lastExpression = null) {
        const t = expr.type;
        if (t === "CallExpression") {
            ...
            if (variable.type === ResultType.RBP_OFFSET) {
                this.emit(`mov DWORD [rbp${positive}${this.s_offset}], [rbp${positive}${variable.value.offset}]`);
            }
            else if (variable.type !== ResultType.LABEL) {
                this.emit(`mov DWORD [rbp${positive}${this.s_offset}], ${variable.value}`);
            }
        }
