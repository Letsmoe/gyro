...
import {Option, ArgumentParser} from "./types";


function makeType(value: string, type: string) {
	if (type === "number") {
		return Number(value);
...
function maybeCapture(opt: Option) {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg.startsWith("--")) {
			...
			if (arg in commands) {
				...
				copyArgs.splice(i, 1);
				commands[arg].callback(colarg(copyArgs));
				break;
			} else {
		...
		for (const key in expected) {
			if (expected.hasOwnProperty(key)) {
				...
				if (element.required && !objArgs[key]) {
					console.log(
						`Missing required argument: --${key} | -${element.alias}`
					);
					process.exit(1);
				}
			}
...
function colarg(argv: string[]) {
	...
	const obj: ArgumentParser = {
		...
		command(
			name: string,
			description: string,
			callback: (parser: ArgumentParser, args: any) => void
		) {
			objCommands[name] = {
				name: name,
		...
		option({ name, alias, type, defaults, description, required, capture }: Option) {
			objOptions[name] = {
				name: name,
				alias: alias,
				type: type,
		...
		help() {
			this.option({name: 'help', description: 'Prints this help message and exits.', required: false, alias: 'h', capture: () => {
				generateHelp(objOptions,objCommands, usageString);
				process.exit(0);
			}});
			return obj;
		},
